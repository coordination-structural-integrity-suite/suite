...
If you are new, start with the **[Where Do I Start? triage prompt](prompts/suite/suite-triage-where-do-i-start-0_1_0.md)**. Copy it into any AI assistant (Claude, ChatGPT, Gemini, or similar), describe your organization and situation in plain language, and it will identify which part of the suite is relevant and route you to the right next step — with an explanation of why. It also produces a findings document you can save and return to as your situation develops.

If you would rather orient yourself in the architecture before describing your situation, the **[Tensegrity Architecture orientation prompt](prompts/tensegrity-architecture/tensegrity-architecture-orientation-0_1_0.md)** is the place to start. It explains why the suite has the shape it does and helps you place your organization within it.

All prompts are in the `prompts/` folder. The **[prompts README](prompts/prompts-README.md)** maps every prompt by type and use case. For extended reasoning work with an AI assistant that already has access to your documents and tools, **Claude skills** are in the `skills/` subfolders — they encode the full reasoning frame for each document and for the suite as a whole.

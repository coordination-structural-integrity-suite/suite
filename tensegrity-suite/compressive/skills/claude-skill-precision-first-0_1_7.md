...
The standard specifies twelve corollaries to the core precision requirement. The core is the double negation: precision fails when violations are undetectable (precision deficit) or when compliance can be demonstrated while the coordination purpose remains unserved (precision imposition). The requirement is met when violations are detectable and compliance can demonstrate that the coordination purpose is being served.

Twelve specific requirements follow from this. Each requirement runs in two directions: a floor condition (what under-specification fails to prevent) and a ceiling condition (what over-specification produces). The floor conditions are listed here; the ceiling conditions are in Section 2 of the standard, paired immediately after each floor condition.
...
11. Every classification whose category membership an independent observer cannot determine from the label and its stated definition alone renders each such category as a typed structure carrying the criteria that individuate it, so membership is determinable from structure and observable evidence rather than from a bare label. Forcing a case that fits no category into the nearest one is itself a harm, so this is a non-harming requirement as much as a precision one. Ceiling: imposing typed structure on a set whose categories a label already determines completely (a closed set with a reliable negative test) adds apparatus without adding precision.
12. The specification states a procedure for its own revision that an independent observer can determine exists: what triggers a revision, who holds standing to initiate one, and how a proposed change is evaluated against the precision criterion. A coordination domain generates its own change, so some way to evolve the specification is invariant rather than optional; a specification with no stated revision procedure lets deficits accumulate until only a disruptive rewrite remains. Ceiling: a revision procedure so rigid or costly that proportional change is foreclosed forces revolution instead of evolution.
...
**Treating precision deficit as the only failure direction.** Over-specification is an equal failure. A system can demonstrate formal compliance with all twelve corollaries while the coordination purpose the instrument was designed to serve remains unreachable for the people it was supposed to protect. If an organization can show it followed every procedure while those procedures were supposed to protect had no access to the protection, the standard is not met.
...
|---------|------|---------|
| 0.1.8 | 2026-09-18 | Corollaries 11 (structural solution over taxonomy) and 12 (stated procedure for a specification's own evolution) added to the requirements list, and the corollary count updated from ten to twelve in the Method-Structure Congruence section, the "what the standard requires" opening, and the common-misreadings section. Follows PFDS v2.5.0. Filename stem left frozen per the skills edit-in-place convention. |

...
This distinction matters for the constituency definition requirement. When a governance system's decisions affect parties who cannot participate — either because they lack current standing or because they do not yet exist — an implicit constituency has been created: parties affected by governance outcomes who are structurally excluded from the governance process. The Distribute invariant requires that this exclusion be named and assessed rather than treated as outside this standard's concern. Where proxy or guardian architecture is present, the authorization for that architecture — who holds the guardian role, on what structural basis, with what accountability to the constituency they represent — is itself a concentration-sensitive structural condition subject to the Authorize invariant.
...
The Distribute, Authorize, and Monitor invariants address power distribution within the adopting system's boundary. Section 4.4 extends those invariants to nested and polycentric governance structures. Neither addresses the class of structural conditions through which external actors can supersede a governance system's internal outcomes regardless of what the governance process produces. External override capacity names this class. A governance system that does not assess its external override exposure operates as though its internal governance outcomes determine actual coordination decisions. When override capacity is present and undisclosed, participants exercise governance that is structurally irrelevant to outcomes. That is a specific harm: the consent architecture is satisfied formally while being materially voided by conditions the governance record does not reflect.

Note on scope boundary: the internal capture path — external actors acquiring internal governance leverage and using governance mechanisms to produce override outcomes — is addressed by the Distribute, Authorize, and Monitor invariants and the change classification architecture in Section 5. Section 4.5 addresses the external-to-internal path: override capacity that resides outside the governance system's boundary and operates regardless of the internal governance outcome. The two paths require different detection architectures and are addressed by distinct sections of this standard.
...
**Class 1: Financial.** Material resource dependency creates override capacity. An actor controlling resources the adopting system requires can impose conditions as the price of access. This class includes: dependency relationships produced by loans, grants, and revenue channels; governance token concentration held by a single actor whose financial interests are structurally distinct from the governed constituency's interests; and treasury structures that give development organizations or legal entities control over financial flows the governance system claims to govern. The condition may be present at founding regardless of intent; token distribution records and financial dependency mapping are the primary audit surfaces. Detection instrument: governance token distribution assessment and resource dependency mapping at each tier review.

**Class 2: Military/coercive.** Credible capacity to apply physical force against the system's actors, infrastructure, or assets creates override capacity independent of legal process. This class includes state and paramilitary actors with jurisdiction over the system's key participants. The override does not require a formal enforcement action; the credible threat is sufficient to produce behavioral override of governance outcomes. Detection instrument: assessment of jurisdictions in which key actors, infrastructure operators, and asset holders are located, and whether those jurisdictions carry coercive authority with respect to the system's governance functions.

**Class 3: Regulatory.** Legal and jurisdictional architecture creates override capacity through treaties, debt obligations, regulatory enforcement, and sovereign authority. This class includes: securities enforcement regimes that classify governance participation or governance token holding as regulated activity; jurisdictional mismatch between the governance system's design and external legal structures; legal entity structures that hold legally enforceable authority over the same resources or decisions the governance system claims to govern; and default legal structures that apply to unincorporated organizations (absence of a recognized legal entity wrapper is a default regulatory exposure, not a neutral condition). The legal entity structure override condition named in Section 3.1 is a Class 3 instance operating within the authority dimension. Section 4.5 extends the regulatory class to all override conditions that operate across the boundary of the governance system, not only those expressed through internal legal entity structures. Detection instrument: assessment of applicable regulatory regimes, contractual obligations with external parties, jurisdiction of entities holding system assets or infrastructure, and confirmation of the system's legal entity status.

**Class 4: Platform.** Infrastructure dependency creates override capacity. A governance system whose operation requires infrastructure it does not control is subject to override by whoever controls that infrastructure. Infrastructure dependency categories include: banking and fiat on/off ramp access; stablecoin issuers and custodied asset holders; smart contract platform operators; distribution channels for governance records or coordination outputs; and technical platform providers whose policy changes can alter the system's operational conditions. Infrastructure dependencies that are established early in a system's lifecycle and locked in through deployed contracts, established integrations, or operational reliance are typically not renegotiable from inside a crisis; the period of greatest structural addressability for this class is before these dependencies become load-bearing. Detection instrument: inventory of infrastructure the system operates on but does not control, with an alternatives landscape assessment for each dependency.

**Class 5: Epistemic.** Control of recognized knowledge frameworks creates override capacity. An actor that controls what counts as legitimate analysis, which frameworks define the governance problem space, and who holds recognized expertise can declare a governance outcome illegitimate within the recognized framework regardless of the governance process's internal legitimacy. The governed system cannot contest the framework itself from within the framework; only an independently held epistemic standing can do so. Detection instrument: identification of external actors whose epistemic frameworks govern how the system's governance outputs are evaluated, and assessment of whether the system's governance architecture is legible and defensible within those frameworks.

**Class 6: Normative.** Control of legitimacy standards creates override capacity independent of epistemic control. An actor that controls what values governance must appeal to, what institutional forms are recognized as legitimate, and what outcomes recognized authority will accept can void governance outcomes on normative grounds without any factual dispute. The normative claim does not need to be correct on the merits to produce override; it needs only to be recognized as legitimate by actors who can enforce the override. Detection instrument: identification of external normative authorities whose legitimacy determinations apply to the system's governance outputs, and assessment of the system's standing to contest normative override when it occurs.

**Class 7: Relational dependency.** The relationship itself is the override mechanism. A single person or organization holds sufficient relational centrality within the governance system that the system cannot proceed, maintain identity, or generate coordination without their participation or recognition. This class is structurally distinct from all prior classes: it requires no infrastructure, no authority structure, and no external framework. The override capacity inheres in the relational topology of the system itself.

Relational dependency may arise from several distinct structural sources: the act of founding (where the relational centrality does not diminish with time or transfer through formal succession); ongoing output that the system continuously depends on and cannot replace through role succession; or community recognition that the entity's identity is inseparable from this actor's participation, regardless of formal governance continuity or legal ownership.
...
**11.6 Relational dependency detection architecture.** The detection instruments for Classes 1 through 6 in Section 4.5 operate primarily on formal records: token distribution data, regulatory filings, infrastructure dependency assessments, and governance records accessible to independent review. Class 7 (relational dependency) requires behavioral pattern analysis of informal coordination records that formal governance records do not capture. The class is named and the structural sources are defined in Section 4.5; the monitoring architecture that would satisfy the Monitor invariant for this class at the SPDS-Instrumented tier is an open design question.

Four behavioral indicators are candidates for instrumentation. (1) Consultation asymmetry: whose input is sought before governance decisions are finalized, as distinct from who formally participates in the ratification step. (2) Framing adoption: whether governance discourse adopts the candidate actor's problem frames as the default starting point, regardless of whether alternative framings are formally available. (3) Decision stall correlation: whether governance decisions reliably stall, reverse, or fail to reach resolution when the candidate actor is absent or withholds endorsement, independent of the formal vote count. (4) Legitimacy attribution: whose participation is cited as the reason a governance outcome is valid or invalid, as a pattern over multiple governance cycles. These four patterns are not captured in formal voting or authorization records; they require longitudinal analysis of coordination forum records, meeting notes, and informal communication channels where these are available.
...
Candidate instrumentation includes: structured analysis of governance forum participation and endorsement patterns; meeting record analysis where coordination sessions are documented; proposal authorship and co-authorship network mapping; and coordination network topology analysis to identify structural centrality that diverges from formal role distribution. The full specification of this detection architecture requires domain-specific pilot data.

**Inheritance.** When this section is resolved — that is, when the monitoring architecture is specified and a pilot dataset establishes the measurement instruments at a level of precision sufficient to satisfy the Monitor invariant — the partial compliance by disclosure exception for Class 7 in Section 4.5's tier provisions is superseded. The SPDS-Instrumented tier's full detection requirement becomes operative for Class 7 without modification to Section 4.5's text. This inheritance is built into Section 4.5's conditional language ("only while Section 11.6 is unresolved"); no amendment to that section is required when this section is resolved. Until resolved, the partial compliance by disclosure provision in Section 4.5's tier provisions applies to Class 7 at the SPDS-Instrumented tier.

# Coordination Structural Integrity Suite - Conflict Transformation Preconditions Readiness Assessment
Use when you are planning to run a conflict transformation process and want to verify that structural preconditions are in place, or when a process has already been deployed and you want to understand why it did not produce the intended result. This prompt walks through the six sequential preconditions and produces a proceed/adjust/defer recommendation for each.

This is not an audit of the Conflict Transformation Standard. It is a pre-deployment readiness check for a specific process.
...
Ask the person to describe the conflict situation and any process design they have developed. Then assess each precondition:
...
**Precondition 3: Process Mode Selection.**
The transformation tradition must fit the situation. Available traditions include: Nonviolent Communication (relational register, interpersonal conflicts), Restorative Justice (harm acknowledgment, community contexts), Deep Democracy (voice for all parts of a system, including marginalized voices), Ho'oponopono (collective reconciliation, community responsibility), and others.

Ask: Does the facilitator hold credentials or genuine practice experience in the selected tradition? Does the tradition fit the relational register of the conflict (some traditions require a degree of relational readiness that may not be present)? Does it fit the organizational context and the parties' developmental capacities?

Assess as: **Proceed**, **Adjust** (select a different mode better matched to context or credentials), or **Defer** (no appropriate mode is available or facilitator credentials are absent).
...
Assess as: **Proceed**, **Adjust** (define the traversal mode and recording conditions before starting), or **Defer** (the design is too incomplete to assess).

# Coordination Structural Integrity Suite - Structural Patterns Primer Recognition Guide
Use when you can describe a recurring dynamic in your organization and want to know what structural pattern it represents, or when you want to understand which standards are most relevant to what you are seeing before deciding where to enter the suite. This prompt takes a description of what is observable and produces a structural pattern identification with the relevant standards.

This is not an audit. There is no score. The output is a structural reading of what you have described.
...
The ten named structural patterns and their observable signatures:
...
3. **Name the standards most relevant.** For each identified pattern, name the standards involved and what each one addresses in this context.

4. **Identify the highest-leverage entry point.** When multiple patterns are active, the standards that appear across all of them are the highest-priority entry points. Name the overlap.

5. **Suggest the next step.** Name the specific next prompt or document: the Suite Deployment Contexts prompt if a clear presenting situation has emerged, the Suite Integration Guide prompt if they are already adopting standards and want to sequence next steps, or a specific standard's audit or capacity development prompt if the pattern points clearly to one standard.

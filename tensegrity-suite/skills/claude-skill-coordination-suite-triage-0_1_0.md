...
**Suite Integration Guide prompt.** For someone already adopting standards who wants to know what to prioritize next, or whose audit results may be affected by detection reliability dependencies. Located in: suite/ prompt folder.

**Tensegrity Architecture prompt.** For someone trying to understand why the suite has the shape it does, or trying to place themselves in the architecture for the first time. Located in: tensegrity-architecture/ prompt folder.

**Structural Patterns Primer prompt.** For someone who can describe a recurring dynamic and wants to know what structural pattern it represents and which standards address it. Located in: structural-patterns-primer/ prompt folder.

**Six-V Trust Framework application guide.** For someone assessing a specific relationship that is advancing in access or trust, or analyzing why a past trust-extension decision failed. Located in: six-v/ prompt folder.
...
**Per-standard compressive audit prompts (six).** For formal compliance assessment against a specific compressive standard. Each produces a failure map by criterion. Use when: the organization has adopted a standard and wants to evaluate structural compliance, or when preparing for external audit. Standards: Precision-First Design Standard, Adverse-Signal Engagement Principle Core Standard, Structural Consent Legibility Standard, Information Asymmetry Classification Standard, Structural Power Obligation Standard, Regenerative Obligation Standard. Some standards have domain-specific variants (general, governance, software for Precision-First; organizational, AI, regulatory for Adverse-Signal Engagement).
...
**Route to the CT Preconditions prompt** when the person is planning to run a conflict transformation process and has not yet assessed whether structural preconditions are in place.

**Route to suite-level prompts** when the person wants a comprehensive suite-wide assessment rather than a starting point.

## Multiple routing

When the triage identifies that more than one prompt is relevant, name all of them in order of priority and explain why each applies. The person can run them in sequence. The findings document should record all recommended prompts and the routing rationale for each.
...
## What the findings document contains

The findings document produced by the triage contains: a brief description of the situation as understood; the structural class identified; the prompt or prompts recommended; the rationale for each routing decision; a timestamp; and a prior-run comparison section (empty on first run, populated when they paste a prior findings document into a subsequent triage). The comparison section notes what has changed in situation, routing, and rationale since the prior run.

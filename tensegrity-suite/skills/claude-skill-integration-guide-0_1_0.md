...
Every organization that claims full Tensegrity Compressive Standards designation without holding it must produce a structural exposure disclosure. The disclosure must name: each absent standard; the failure mode class it addresses; a self-assessed exposure level with rationale; any compensating controls; and the detection reliability bounds of any audits produced under partial adoption.
